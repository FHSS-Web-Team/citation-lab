...
export class CitationTemplate {
  ...
  wrapExpression(startIndex: number, endIndex: number) {
    ...
    const copy = [...this.template];
    const replacement = new CitationTemplateExpression(
      copy.slice(startIndex, endIndex + 1)
    );
    copy.splice(startIndex, endIndex - startIndex + 1, replacement);
  ...
  setTemplate(
    nextParts: CitationTemplateAddend[],
    ...
    if (resetHistory) {
      ...
      this.history.push(this.getParts());
    }
